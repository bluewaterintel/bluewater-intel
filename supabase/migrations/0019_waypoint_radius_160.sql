-- Extend charted waypoint query radius cap from 120 nm to 160 nm so Gulf ports
-- (e.g. Naples → Tampa Bay / Pulley Ridge runs) can load the full local set.

create or replace function public.bw_pack_radius_nm() returns integer
language sql immutable as $$ select 160 $$;
